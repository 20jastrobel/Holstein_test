...
# HH seed-surface comparison for one point:
python pipelines/exact_bench/cross_check_suite.py --problem hh --L 2 \
  --omega0 1.0 --g-ep 0.8 --n-ph-max 1 \
  --hh-seed-refine-surface

# HH seed benchmark preset:
python pipelines/exact_bench/cross_check_suite.py --problem hh \
  --hh-seed-benchmark-preset mini4
```
...
Opt-in HH seed-refine surface (`--hh-seed-refine-surface`):
- Keeps the default HH matrix above, then additively appends conventional pre-ADAPT seed arms:
  - `HH-PhysicalTermwise`
...
- `--hh-seed-benchmark-preset mini4` runs the small HH comparison set
  `L ∈ {2,3}`, `g ∈ {0.8,1.2}`, `t=1`, `U=4`, `ω0=1`, `n_ph_max=1`.
- Primary decision metric for this surface is energy improvement per added `cx_proxy`;
  secondary views are improvement per added `depth_proxy` and `sq_proxy`.
- Keep proxy formulas sourced from `docs/reports/qiskit_circuit_report.py` via the
  existing sidecar utilities; do not duplicate those formulas elsewhere.
...
### Hardware/circuit honesty rule
- True numerical CFQM4/CFQM6 means `--cfqm-stage-exp expm_multiply_sparse` or `--cfqm-stage-exp dense_expm`.
- These numerical stage backends are statevector/numerical-only in the current repo; they are **not** honest compiled/transpiled/CX/QPU surfaces.
- `--cfqm-stage-exp pauli_suzuki2` is the only circuitizable CFQM profile for report/transpile/hardware validation. It is a circuitized surrogate and must not be described as true CFQM4/CFQM6 or true compiled CFQM4/CFQM6.
- If a doc, report, artifact, or agent response says `compiled/transpiled CFQM4 circuit`, it must qualify that this means the `pauli_suzuki2` surrogate unless it is explicitly saying circuitization is unsupported/skipped.
- If the user asks for both true numerical CFQM4/CFQM6 and an honest CX/QPU/transpile budget, agents must state that these requirements conflict in the current repo and ask which surface they want.
...
- Honest hardware-style budgets apply only to the circuitized `pauli_suzuki2` surrogate path; do **not** assign honest CX/QPU budgets to `dense_expm` or `expm_multiply_sparse`.
- Agents must distinguish these budget surfaces explicitly:
  - **full-trajectory propagator budget** = one compiled/transpiled dynamics circuit to `t_final`
  - **snapshot propagator budget** = max per-sampled-time compiled/transpiled dynamics circuit when each sampled `t_i` is a separate job
  - **checkpoint-fit / local-fit budget** = max per-snapshot fitted surrogate circuit; this is **not** a propagator budget
- `pipelines/exact_bench/hh_fixed_seed_qpu_prep_sweep.py --budget-mode full_trajectory` and `--budget-mode snapshot` are both propagator surfaces; `pipelines/exact_bench/hh_fixed_seed_local_checkpoint_fit.py` is a per-snapshot surrogate surface and must not be described as Suzuki/CFQM propagation.
- Snapshot/job budgets are often the more practical QPU execution surface because sampled times are separate jobs, but they do **not** turn checkpoint-fit/local-fit surrogates into honest propagators.
- For Suzuki/CFQM propagators, switching from `full_trajectory` to `snapshot` budget semantics does **not** automatically lower max CX/depth; when every sampled snapshot uses the same macro-step count, the max propagated circuit can stay equal to the `t_final` circuit. The main change is budget interpretation/job decomposition, not guaranteed gate reduction.
- If a user compares checkpoint-fit CX counts (for example 20/40/60 CX) against full-trajectory or snapshot propagator CX counts, agents must call out that these numbers come from different circuit families and different cost definitions.
...
6. The staged HH path may optionally insert a conventional explicit-family
   seed-refine stage between warm-start and ADAPT.
   - Default remains warm `hh_hva_ptw` -> ADAPT(`phase3_v1`) -> matched-family replay.
   - When refine is enabled, the bundle may add `seed_provenance`; missing
     `seed_provenance` must remain readable and means “no refine stage”.
   - Replay family resolution remains `match_adapt` with `full_meta` fallback
     and must ignore `seed_provenance`.
