...
UNICODE_COMMANDS = {
  ...
  'uparrow': '↑', 'downarrow': '↓',
  'langle': '⟨', 'rangle': '⟩', 'lvert': '|', 'rvert': '|', 'dagger': '†',
...
def tex2text(x, options):
  ...
  x = re.sub(r'\\if(full|false)\s*(.*?)\\fi', '', x, flags=re.DOTALL)
  x = re.sub(r'\n\n+', r'<P>', x)
  x = re.sub(r'\s+', r' ', x)
  x = re.sub(r'<P>', r'\n\n', x)
  ...
  if options.math:
    ...
    # Strip common TeX font wrappers so names survive command cleanup.
    while True:
      y = re.sub(r'\\mathrm\s*{([^{}]*)}', r'\1', x)
      if y == x:
        break
      x = y
    ...
    if options.unicode:
      ...
      x = re.sub(r'\\lvert(?![A-Za-z])', r'|', x)
      x = re.sub(r'\\rvert(?![A-Za-z])', r'|', x)
      ...
      def unicode_command(match):
        return UNICODE_COMMANDS.get(match.group(1), match.group(0))
      x = re.sub(r'\\([A-Za-z]+)(?![A-Za-z])', unicode_command, x)
      ...
      x = re.sub(r'\\lvert(?![A-Za-z])', r'|', x)
      x = re.sub(r'\\rvert(?![A-Za-z])', r'|', x)
      x = re.sub(r'\\langle(?![A-Za-z])', r'<', x)
      x = re.sub(r'\\rangle(?![A-Za-z])', r'>', x)
      x = re.sub(r'\\dagger(?![A-Za-z])', r'^*', x)
      x = re.sub(r'\\prod(?![A-Za-z])', r'prod', x)
      x = re.sub(r'\s*\\leq?\b\s*', r' <= ', x)
      x = re.sub(r'\s*\\geq?\b\s*', r' >= ', x)
      x = re.sub(r'\s*\\neq?\b\s*', r' != ', x)
