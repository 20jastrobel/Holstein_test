...
<user_instructions>
<taskname="Beam ADAPT Refactor Contract"/>
<task>
Produce a repo-grounded refactor/design contract, not code, for adding beam search to the HH staged ADAPT controller. Beam search here means B independent live partial ansatz branches in parallel. Preserve current behavior exactly when beam width `B=1`.

Do not treat beam search as “append several generators to one live circuit”. The key change is controller/search-layer state management, not the quantum measurement primitive.
</task>

<decisive_architecture>
The decisive repo fact is not just top-1 selection. `pipelines/hardcoded/adapt_pipeline.py::_run_hardcoded_adapt_vqe` currently conflates three roles into one mutable trajectory:
1. run-global context — pool, Hamiltonian, shared static caches, config, logger
2. branch-local state — operator sequence, theta, optimizer memory, selection counts, available indices, stage telemetry
3. step-local scratch — shortlist rows, candidate variants, scaffold contexts, temporary audits

Beam search should not be implemented until those are decomposed into reusable branch-step units. Otherwise beam becomes a fragile copy/paste fork of the current loop.
</decisive_architecture>

<confirmed_repo_facts>
- `adapt_pipeline._run_hardcoded_adapt_vqe` currently mutates one live `selected_ops`, one `theta`, one `history`, one `available_indices`, and one `phase2_optimizer_memory` object.
- `adapt_pipeline` currently collapses phase-2 admission to a singleton via `phase2_selected_records = [eligible_full_records[0]]`.
- `pipelines/hardcoded/hh_continuation_scoring.py::greedy_batch_select` currently returns top-1 only.
- Runtime-split shortlist children are also collapsed back to one kept variant per parent via `candidate_variants[0]`.
- Replay/export are single-trajectory: `handoff_state_bundle.write_handoff_state_bundle` emits one `adapt_vqe.operators` list and one `adapt_vqe.optimal_point`; `hh_vqe_from_adapt_family._extract_adapt_operator_theta_sequence` and `build_replay_ansatz_context` consume one trajectory.
- Tests explicitly lock singleton semantics: `test/test_adapt_vqe_integration.py` asserts `selected_positions == [selected_position]` and `batch_selected is False`.
</confirmed_repo_facts>

<nonnegotiable_design_rules>
1. Do not store `selected_executor` as authoritative branch state.
   Treat it as a derived artifact from branch structure (`selected_ops`, `theta`) and either rebuild on demand or memoize behind a branch fingerprint.

2. Separate immutable shared caches from mutable branch overlays.
   - Shared immutable caches: compiled polynomial/Pauli actions and other data keyed only by static structure.
   - Branch-local mutable overlays: measurement reuse credit, scaffold contexts, temporary audits, branch-local optimizer memory, and any reuse state whose meaning depends on trajectory history.
   Shared caches may be read globally; mutable reuse state must be committed only into branch-local overlays.

3. Make the beam pruning objective explicit.
   Recommended safe v1 choice:
   - within-branch candidate generation/ranking may use existing heuristic scores (`simple_score`, `full_v2_score`, novelty/curvature/compatibility)
   - global beam pruning after expansion should rank children primarily by post-admission, post-reoptimization branch energy
   - heuristic/controller scores should be secondary tie-breakers, not the final beam truth
</nonnegotiable_design_rules>

<v1_boundary>
Beam v1 should include:
- explicit run-global vs branch-local vs step-local decomposition
- branch-local copies of mutable continuation state
- unchanged shared quantum primitives (`CompiledAnsatzExecutor`, compiled polynomial energy path, `vqe_minimize`, scoring oracles)
- exact `B=1` semantic preservation
- global pruning after branch-local admit + reopt
- optional exact structural dedup of children
- winner-only handoff/export using the current schema

Beam v1 should not include:
- live Pareto frontier pruning
- replay/handoff schema changes
- multi-winner export
- cross-branch optimizer-memory sharing
- aggressive branch-parallel runtime-split pool mutation
</v1_boundary>

<repo_touchpoints>
Focus on these symbols/files:
- `pipelines/hardcoded/adapt_pipeline.py`: `_run_hardcoded_adapt_vqe`, `_commutator_gradient`, `_adapt_energy_fn`, `_resolve_reopt_active_indices`, `_splice_candidate_at_position`, `_predict_reopt_window_for_position`
- `pipelines/hardcoded/hh_continuation_scoring.py`: `build_candidate_features`, `build_full_candidate_features`, `shortlist_records`, `CompatibilityPenaltyOracle`, `greedy_batch_select`
- `pipelines/hardcoded/hh_continuation_types.py`: `CandidateFeatures`, `Phase2OptimizerMemoryAdapter`, `ScaffoldFingerprintLite`, replay/telemetry dataclasses
- `pipelines/hardcoded/hh_continuation_stage_control.py`
- `pipelines/hardcoded/hh_continuation_pruning.py`
- `pipelines/hardcoded/hh_continuation_replay.py`: `RestrictedAnsatzView`, replay controller
- `pipelines/hardcoded/hh_staged_workflow.py`: `_handoff_continuation_meta`, `_build_replay_contract`, `_write_adapt_handoff`
- `pipelines/hardcoded/handoff_state_bundle.py`: winner-only export contract
- `pipelines/hardcoded/hh_vqe_from_adapt_family.py`: `_extract_adapt_operator_theta_sequence`, `build_replay_ansatz_context`, `run`
- `src/quantum/compiled_ansatz.py`, `src/quantum/compiled_polynomial.py`, `src/quantum/vqe_latex_python_pairs.py`
</repo_touchpoints>

<deliverables>
Return a high-level refactor contract with these concrete outputs:
1. The minimal helper decomposition needed to break `_run_hardcoded_adapt_vqe` into reusable branch-step units.
2. An explicit `BeamBranchState` schema, identifying authoritative fields vs derived artifacts.
3. Ownership rules for each cache/audit structure: shared immutable cache vs branch-local mutable overlay vs step-local scratch.
4. The beam controller API and how `B=1` maps exactly to current behavior.
5. The exact global beam pruning objective and tie-break ordering.
6. How existing multi-append, shortlist, novelty/curvature, and windowed/local reoptimization should interact with beam.
7. A staged implementation order focused first on monolith decomposition, then branch state, then multi-child expansion, then global pruning, then winner-only export hardening.
8. The precise B=1 invariants and additional tests to lock down branch isolation, order-independence, dedup correctness, and winner-only replay compatibility.

The next artifact should be a refactor contract, not code.
Do not implement.
</deliverables>
</user_instructions>
