...
def _build_hh_poly_and_matrix():
    ...
    qpb = boson_qubits_per_site(_N_PH_MAX, _BOSON_ENCODING)
...
class TestHHTrotterSmoke(unittest.TestCase):
    ...
    def test_hh_vqe_and_sector_filter(self):
        h_poly, coeff_map, nq_total, qpb, hmat = _build_hh_poly_and_matrix()
        ...
        psi_ref = hubbard_holstein_reference_state(
            ...
            boson_encoding=_BOSON_ENCODING, indexing=_ORDERING,
        )
        ...
        # VQE searches full Hilbert space (not sector-constrained), so it
        # may find energies below the sector-filtered ground state.  We
        # only check that it converges to *some* finite energy and that
        # the full-space variational bound holds (VQE >= min eigenvalue).
        eigs_full = np.linalg.eigvalsh(hmat)
        self.assertGreaterEqual(vqe_res.energy, float(eigs_full[0]) - 1e-6)

        # Verify initial VQE state has valid fidelity with ground manifold.
        # VQE is not sector-constrained so overlap may be zero.
...
class TestHHToHubbardReduction(unittest.TestCase):
    ...
    def test_reduction_spectrum(self):
        ...
        np.testing.assert_allclose(
            ...
            evals_hub_repeated,
            atol=1e-10,
            err_msg="HH(omega0=0, g=0) spectrum must embed Hubbard ⊗ I_phonon",
        )
...
class TestSectorFilteringHH(unittest.TestCase):
    ...
    def test_sector_count_blocked(self):
        """Blocked ordering: HH sector count = Hubbard-sector × phonon-configs."""
        qpb = boson_qubits_per_site(_N_PH_MAX, _BOSON_ENCODING)
        nq_total = 2 * _L + _L * qpb
        ...
        # Pure Hubbard sector size: C(L, n_up) * C(L, n_dn)
        ...
        self.assertEqual(
            sector_hh.size,
            expected_count,
            f"HH sector should have {expected_count} states "
    ...
    def test_fermion_only_mask(self):
        ...
        for idx in sector:
            ...
            self.assertEqual(n_up, _NUM_PARTICLES[0],
                             f"State {idx}: n_up={n_up} != {_NUM_PARTICLES[0]}")
            self.assertEqual(n_dn, _NUM_PARTICLES[1],
                             f"State {idx}: n_dn={n_dn} != {_NUM_PARTICLES[1]}")
...
class TestHHExactGroundEnergy(unittest.TestCase):
    ...
    def test_gs_energy_matches_direct(self):
        ...
        gs_api = exact_ground_energy_sector_hh(
            ...
            num_particles=_NUM_PARTICLES,
            n_ph_max=_N_PH_MAX,
            boson_encoding=_BOSON_ENCODING,
            indexing=_ORDERING,
        )

        self.assertAlmostEqual(
            gs_direct, gs_api, places=8,
            msg="Pipeline _ground_manifold and exact_ground_energy_sector_hh must agree",
        )
