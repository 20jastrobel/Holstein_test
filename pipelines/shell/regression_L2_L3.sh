...
"${PYTHON_BIN}" pipelines/hardcoded/hubbard_pipeline.py \
  --L 2 \
  --t "${T_VAL}" \
  --u "${U_VAL}" \
  --dv "${DV_VAL}" \
  ...
  --output-json "artifacts/json/hc_hubbard_L2_static_t${T_VAL}_U${U_VAL}_S${L2_TROTTER_STEPS}.json" \
  --output-pdf "artifacts/pdf/hc_hubbard_L2_static_t${T_VAL}_U${U_VAL}_S${L2_TROTTER_STEPS}.pdf" \
  --skip-pdf

echo "Running L=2 qiskit pipeline..."
"${PYTHON_BIN}" pipelines/qiskit_archive/qiskit_baseline.py \
  --L 2 \
  --t "${T_VAL}" \
  --u "${U_VAL}" \
  --dv "${DV_VAL}" \
  ...
  --qpe-seed "${QPE_SEED}" \
  --initial-state-source vqe \
  --output-json "artifacts/json/qk_hubbard_L2_static_t${T_VAL}_U${U_VAL}_S${L2_TROTTER_STEPS}.json" \
  --output-pdf "artifacts/pdf/qk_hubbard_L2_static_t${T_VAL}_U${U_VAL}_S${L2_TROTTER_STEPS}.pdf" \
  --skip-pdf
...
"${PYTHON_BIN}" pipelines/hardcoded/hubbard_pipeline.py \
  ...
  --qpe-seed "${QPE_SEED}" \
  --initial-state-source vqe \
  --output-json "artifacts/json/hc_hubbard_L3_static_t${T_VAL}_U${U_VAL}_S${L3_TROTTER_STEPS}.json" \
  --output-pdf "artifacts/pdf/hc_hubbard_L3_static_t${T_VAL}_U${U_VAL}_S${L3_TROTTER_STEPS}.pdf" \
  --skip-pdf
...
"${PYTHON_BIN}" pipelines/qiskit_archive/qiskit_baseline.py \
  ...
  --qpe-seed "${QPE_SEED}" \
  --initial-state-source vqe \
  --output-json "artifacts/json/qk_hubbard_L3_static_t${T_VAL}_U${U_VAL}_S${L3_TROTTER_STEPS}.json" \
  --output-pdf "artifacts/pdf/qk_hubbard_L3_static_t${T_VAL}_U${U_VAL}_S${L3_TROTTER_STEPS}.pdf" \
  --skip-pdf
...
"${PYTHON_BIN}" pipelines/qiskit_archive/compare_hc_vs_qk.py \
  --l-values 2 \
  --no-run-pipelines \
  --trotter-steps "${L2_TROTTER_STEPS}" \
  --t "${T_VAL}" --u "${U_VAL}" \
...
overall_pass=1
if [[ ! -f artifacts/json/cmp_hubbard_bundle_summary.json ]]; then
  overall_pass=0
else
  compare_all_pass="$("${PYTHON_BIN}" - <<'PY'
import json
from pathlib import Path
path = Path("artifacts/json/cmp_hubbard_bundle_summary.json")
...
for tag in "${L2_TAG}" "${L3_TAG}"; do
  if "${PYTHON_BIN}" reports/compare_jsons.py \
      --hardcoded "artifacts/json/hc_hubbard_${tag}.json" \
      --qiskit "artifacts/json/qk_hubbard_${tag}.json" \
      --metrics "artifacts/json/cmp_hubbard_${tag}_metrics.json"; then
...
DRIVE_TAG_L2="L2_drive_t${T_VAL}_U${U_VAL}_S${DRIVE_TROTTER_STEPS}"
DRIVE_TAG_L3="L3_drive_t${T_VAL}_U${U_VAL}_S${DRIVE_TROTTER_STEPS}"
...
"${PYTHON_BIN}" pipelines/hardcoded/hubbard_pipeline.py \
  ...
  --output-json "${DRIVE_ARTIFACTS_DIR}/json/hc_hubbard_${DRIVE_TAG_L2}.json" \
  --output-pdf "${DRIVE_ARTIFACTS_DIR}/pdf/hc_hubbard_${DRIVE_TAG_L2}.pdf" \
  --skip-pdf
...
"${PYTHON_BIN}" pipelines/qiskit_archive/qiskit_baseline.py \
  ...
  --drive-pattern dimer_bias \
  --drive-time-sampling "${DRIVE_TIME_SAMPLING}" \
  --output-json "${DRIVE_ARTIFACTS_DIR}/json/qk_hubbard_${DRIVE_TAG_L2}.json" \
  --output-pdf "${DRIVE_ARTIFACTS_DIR}/pdf/qk_hubbard_${DRIVE_TAG_L2}.pdf" \
  --skip-pdf
...
"${PYTHON_BIN}" pipelines/hardcoded/hubbard_pipeline.py \
  ...
  --drive-time-sampling "${DRIVE_TIME_SAMPLING}" \
  --output-json "${DRIVE_ARTIFACTS_DIR}/json/hc_hubbard_${DRIVE_TAG_L3}.json" \
  --output-pdf "${DRIVE_ARTIFACTS_DIR}/pdf/hc_hubbard_${DRIVE_TAG_L3}.pdf" \
  --skip-pdf
...
"${PYTHON_BIN}" pipelines/qiskit_archive/qiskit_baseline.py \
  ...
  --drive-pattern staggered \
  --drive-time-sampling "${DRIVE_TIME_SAMPLING}" \
  --output-json "${DRIVE_ARTIFACTS_DIR}/json/qk_hubbard_${DRIVE_TAG_L3}.json" \
  --output-pdf "${DRIVE_ARTIFACTS_DIR}/pdf/qk_hubbard_${DRIVE_TAG_L3}.pdf" \
  --skip-pdf
...
for L in 2 3; do
  DTAG="L${L}_drive_t${T_VAL}_U${U_VAL}_S${DRIVE_TROTTER_STEPS}"
  if "${PYTHON_BIN}" reports/compare_jsons.py \
      --hardcoded "${DRIVE_ARTIFACTS_DIR}/json/hc_hubbard_${DTAG}.json" \
      --qiskit "${DRIVE_ARTIFACTS_DIR}/json/qk_hubbard_${DTAG}.json" \
      --metrics "${DRIVE_ARTIFACTS_DIR}/json/cmp_hubbard_${DTAG}_metrics.json"; then
...
gate_pass = True
for L in (2, 3):
    tag  = f"L{L}_drive_t{t_val}_U{u_val}_S{steps}"
    path = artifacts_dir / "json" / f"hc_hubbard_{tag}.json"
    if not path.exists():
