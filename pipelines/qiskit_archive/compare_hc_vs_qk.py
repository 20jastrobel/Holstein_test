...
# ---------------------------------------------------------------------------
# Path setup — this file lives at pipelines/qiskit_archive/compare_hc_vs_qk.py
# ---------------------------------------------------------------------------
...
def _artifact_tag(args: argparse.Namespace, L: int) -> str:
    ...
    Format: ``L{L}_{drive|static}_t{t}_U{u}_S{trotter_steps}``

    Examples:
        ``L2_static_t1.0_U4.0_S32``
        ``L3_drive_t1.0_U4.0_S64``
    """
    mode = "drive" if getattr(args, "enable_drive", False) else "static"
...
def _write_comparison_pdf(
    *,
    ...
    with PdfPages(str(pdf_path)) as pdf:
        ...
        render_text_page(pdf, summary_lines, fontsize=10, line_spacing=0.029, max_line_width=112)
...
def _run_amplitude_comparison_for_l(
    ...
    ``amp_hc_hubbard_`` / ``amp_qk_hubbard_`` prefix so they do not overwrite the main
    pipeline outputs.
    """
    ...
    def _base_cmd_qk(json_out: Path) -> list[str]:
        cmd = _base_cmd("pipelines/qiskit_archive/qiskit_baseline.py", json_out, include_hardcoded_ansatz=False)
    ...
    for label, drive_tokens in [
        ...
        hc_json = json_dir / f"amp_hc_hubbard_{tag}_{slug}.json"
        qk_json = json_dir / f"amp_qk_hubbard_{tag}_{slug}.json"
...
def _write_amplitude_comparison_pdf(
    ...
    with PdfPages(str(pdf_path)) as pdf:
        ...
        render_text_page(pdf, settings_lines)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "pipelines/hardcoded/hubbard_pipeline.py --problem hh."
        ),
    )
    ...
    # Defaults mirror pipelines/hardcoded/hubbard_pipeline.py parse_args().
...
def main() -> None:
    ...
    if getattr(args, "problem", "hubbard") == "hh":
        raise SystemExit(
            ...
            "  python pipelines/hardcoded/hubbard_pipeline.py --problem hh [flags …]"
        )
    ...
    # process (CWD may differ) and subprocesses (cwd=REPO_ROOT).
    ...
    for L in l_values:
        ...
        tag = _artifact_tag(args, L)
        qk_json = json_dir / f"qk_hubbard_{tag}.json"
        qk_pdf = pdf_dir / f"qk_hubbard_{tag}.pdf"
        metrics_json = json_dir / f"cmp_hubbard_{tag}_metrics.json"
        hc_json_by_ansatz: dict[str, Path] = {}
        hc_pdf_by_ansatz: dict[str, Path] = {}
        compare_pdf_by_ansatz: dict[str, Path] = {}
        for ansatz_name in ordered_hardcoded_ansatzes:
            if ansatz_name == primary_hardcoded_ansatz:
                hc_json = json_dir / f"hc_hubbard_{tag}.json"
                hc_pdf = pdf_dir / f"hc_hubbard_{tag}.pdf"
                compare_pdf = pdf_dir / f"cmp_hubbard_{tag}.pdf"
            else:
                hc_json = json_dir / f"hc_hubbard_{ansatz_name}_{tag}.json"
                hc_pdf = pdf_dir / f"hc_hubbard_{ansatz_name}_{tag}.pdf"
                compare_pdf = pdf_dir / f"cmp_hubbard_{ansatz_name}_{tag}.pdf"
    ...
    isolation_check = _check_hardcoded_qiskit_import_isolation(REPO_ROOT / "pipelines" / "hardcoded" / "hubbard_pipeline.py")
    ...
    summary_json_path = json_dir / "cmp_hubbard_bundle_summary.json"
    summary_json_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    bundle_pdf_path: Path | None = None
    if not args.skip_pdf:
        bundle_pdf_path = pdf_dir / "cmp_hubbard_bundle.pdf"
        _write_bundle_pdf(
    ...
    if bool(getattr(args, "with_drive_amplitude_comparison_pdf", False)):
        ...
        for L in l_values:
            ...
            tag = _artifact_tag(args, L)
            amp_pdf_path = pdf_dir / f"amp_cmp_hubbard_{tag}.pdf"
            ...
            amp_metrics_path = json_dir / f"amp_cmp_hubbard_{tag}_metrics.json"
