...
def _vqe_ground_energy(
    ...
    *,
    ...
    reps: int = 1,
    term_sort: str = "magnitude",
    normalize_term_coeff: bool = True,
) -> float:
    ...
    if ansatz_style == "termwise":
        ansatz = _termwise_ansatz(
            H,
            reps=reps,
            term_sort=term_sort,
            normalize_term_coeff=normalize_term_coeff,
        )
...
def _termwise_ansatz(H, *, reps: int = 1, term_sort: str = "input", normalize_term_coeff: bool = False):
    ...
    term_polys = [
        PauliPolynomial(
            repr_mode,
            [PauliTerm(nq, ps=term.pw2strng(), pc=1.0 if normalize_term_coeff else complex(term.p_coeff))],
        )
        for term in terms
    ]
