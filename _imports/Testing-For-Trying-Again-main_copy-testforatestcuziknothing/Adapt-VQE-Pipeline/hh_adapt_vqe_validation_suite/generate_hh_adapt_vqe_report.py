...
def _pool_lookup(settings: dict[str, Any], cache: dict[tuple[Any, ...], dict[str, Any] | None]) -> dict[str, Any] | None:
    ...
    try:
        pool_key = str(settings.get("adapt_pool", "hva")).strip().lower()
        if pool_key in {"paop", "paop_min", "paop_std", "paop_full"}:
            l_value = int(settings.get("L", 0))
            num_particles = ((l_value + 1) // 2, l_value // 2)
            pool = hardcoded_adapt._build_paop_pool(
                ...
                pool_key=pool_key,
                paop_r=int(settings.get("paop_r", 0)),
